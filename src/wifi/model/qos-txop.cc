...
void QosTxop::NotifyInternalCollision (void)
{
  ...
  if (!GetMuMode ())
   {
    GenerateBackoff ();
   }//Xyct: stub are left in convenience of later development
  else
   {
    GenerateBackoff ();
   }
...
QosTxop::GotAck (void)
{
  ...
  if (!m_currentIsFragmented
    ...
    {//Xyct: disable hacking backoff trace to 0: TF should not affect txop
