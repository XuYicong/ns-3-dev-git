...
public:
  ...
  void SetTfDuration (uint32_t tfDuration); //Xyct: TODO: move these to mac low
...
protected:
  ...
  Ptr<ChannelAccessManager> m_channelAccessManagerMu [9]; //!< channel access manager
  Ptr<WifiPhy> m_phy;           //!< Wifi PHY
  Ptr<WifiPhy> m_phyMu [9]; 
  double m_alpha;
  uint32_t m_ruBits;
  bool m_muMode;
  uint32_t m_tfDuration;//Xyct: TODO: move these to mac low
  ...
  Ptr<Txop> GetMuTxop (uint32_t i) const;
  ...
  Ptr<QosTxop> GetMuVOQueue (uint32_t i) const;
  /**
  ...
  Ptr<QosTxop> GetMuVIQueue (uint32_t i) const;
  /**
  ...
  Ptr<QosTxop> GetMuBEQueue (uint32_t i) const;
  /**
  ...
  Ptr<QosTxop> GetMuBKQueue (uint32_t i) const;
  ...
  uint32_t m_nScheduled;            //!< Xyct: TODO: move them to mac low
