...
{
  std::map<AcIndex, Ptr<QosTxop> >::const_iterator edcaIt = m_edca.find (AC_BE); //Ideally, I must get tid from pkt and header, and map tid to ac
  ...
  if (hdr.IsQosData () && !hdr.GetAddr1 ().IsGroup ()
      && !hdr.IsMoreFragments () && hdr.GetFragmentNumber () == 0)
    {
      ...
      if (m_mpduAggregator != 0)
        {//Xyct: the commented line was invocating "IsAmpdu", translated by guessing original purpose
          //mpduList = m_mpduAggregator->GetNextAmpdu (newMpdu, m_currentTxVector, txopLimit);// Hack: disabled now to disable aggregation for now
        }
...
void
MacLow::SetTfCwMax(uint32_t q)
{
    m_tfCwMax = q;
}
uint32_t
MacLow::GetTfCwMax(void)
{
    return m_tfCwMax;
}
void
MacLow::SetTfCwMin(uint32_t q)
{
    m_tfCwMin = q;
}
uint32_t
MacLow::GetTfCwMin(void)
{
    return m_tfCwMin;
}

void 
MacLow::SetRuBits (uint32_t ruBits)
{
  m_ruBits = ruBits;
}

uint32_t 
MacLow::GetRuBits (void) const
{
  return m_ruBits;
}

void
MacLow:: StaStopMuMode(void)
{
  NS_LOG_UNCOND ("Inside MacLow:Stopping Sta MuMode, currently do nothing, time = "<<Simulator::Now ().GetMicroSeconds ());
}
void
MacLow::SendTriggerFrame(bool flag, RegularWifiMac::RUAllocations alloc)
{
  NS_LOG_FUNCTION (this);
  /*
     ...
     if (!flag)
      {
        ...
        Simulator::ScheduleNow (&MacLow::StopMuMode, this);
        return; 
      }
   }*/
  if (!flag)
  ...
  //CalculateTfDuration ();
  MgtTFHeader tf;
  tf.SetNoSta (alloc.size ());
  tf.SetRUAllocations (alloc);
  tf.SetUplinkFlag (flag);
  //Xyct: TODO: calculate duration
  Time total=CalculateTfBeaconDuration (packet, hdr)+MicroSeconds(2021);
  std::cout<<"total = "<<total.GetMicroSeconds () << std::endl;
  uint32_t tfDuration = total.GetMicroSeconds () / GetSlotTime ().GetMicroSeconds ();
  tf.SetTfDuration (tfDuration);
  packet->AddHeader (tf);
  //packet->Print(std::cout);
  std::cout<<"Sending TF, time = "<< Simulator::Now ().GetMicroSeconds ()<<std::endl;
  std::cout<<"Number of STAs allocated = " << alloc.size () <<std::endl;
  //std::cout<<"Number of STAs associated = " << m_staList.size () <<std::endl;
  for (RegularWifiMac::RUAllocations::iterator it = alloc.begin (); it!=alloc.end (); ++it)
   {
...
MacLow::CacheDataPacket(){
  ...
  packet->AddHeader (bsrAck);
    //As a MU DL ppdu, needs to send at once
  std::cout<<"Cacheing BSR ACK, time = "<<Simulator::Now ().GetMicroSeconds () << std::endl;  
...
MacLow::SendMuPacketUl(){
  ...
  WifiTxVector txVector = m_currentTxVector;//GetDataTxVector(Create<WifiMacQueueItem>(psdu->GetPayload(0),psdu->GetHeader(0)));
  ...
  HeRu::RuSpec ru;
    ...
    ru.primary80MHz = ru.index<= HeRu::GetNRus(80,ru.ruType);
  ...
  Time txDuration = m_phy->CalculateTxDuration (psdu->GetSize (), txVector, m_phy->GetPhyBand (), txStaId);
  txVector.SetLength (m_phy->ConvertHeTbPpduDurationToLSigLength (txDuration, m_phy->GetPhyBand ()));
  ForwardDown(psdu, txVector);
}
void
...
{
    MacLow::m_summarizeTfCycleCallback=callback;
}
...
{
  std::cout<<"Inside node "<<m_phy->GetDevice ()->GetNode ()->GetId () <<"\tru = "<<index<<"\tXyct try to\tSendTriggerFrameResp\n";
  NS_LOG_FUNCTION (this);
  WifiMacHeader hdr;
  ...
  //Addsomething here
  resp.SetData (CalculateStaPayloadDuration());
  //Data(aka BSR) here always 0, but this value not used
  resp.SetRu (index);
...
MacLow::ReceiveOk (Ptr<WifiMacQueueItem> mpdu, RxSignalInfo rxSignalInfo, WifiTxVector txVector, bool ampduSubframe)
{
  ...
  if (hdr.IsRts ())
    {
      ...
      if (ampduSubframe)
        ...
        {
          if (isPrevNavZero
              && (hdr.GetAddr1 () == m_self || hdr.GetAddr1 ().IsGroup()))
            {
                if(hdr.GetAddr1 ().IsGroup())
                {
                    NS_LOG_DEBUG ("rx MU-RTS from=" << hdr.GetAddr2 () << ", schedule CTS");
                    //1. MU-RTS/CTS when uplink, will there be a second trigger frame? 
                    //2. When trigger frame collides, will the contention window double? NO
                }
                else
                {
           ...
           && m_currentPacket != 0)
    {
      ...
      if(m_currentPacket->GetAddr1 (). IsGroup())
      {
        m_stationManager->ReportRtsOk (from, &m_currentPacket->GetHeader (0),
  ...
  else if(hdr.IsTFResponse() ){//Xyct: received by AP
          MgtTFRespHeader resp;
          packet->RemoveHeader (resp);
          //Xyct: assume all TFresp Rx have been scheduled now
        //uint16_t staId=txVector.GetHeMuUserInfoMap().begin()->first;
          //Simulator::ScheduleNow (&MacLow::SendCachedPacketDl, this, from, resp.GetRu (), staId);//Xyct: TODO: Downlink data
  }
    ...
    if(GetBssid() != from){
      ...
      /*Time m_muDlModeEnd = tf.GetTfDuration () * GetSlotTime ()- m_lastTfTxStart; 
      //m_channelAccessManager->NotifyMaybeCcaBusyStartNow (m_muDlModeEnd); 
      m_muModeExpireEvent = Simulator::Schedule (m_muDlModeEnd, &MacLow::StaStopMuMode, this); */
      ...
      for (RegularWifiMac::RUAllocations::const_iterator it = alloc.begin (); it != alloc.end (); it++)
       {
		 ...
		 for(auto m_channelAccessManager:m_channelAccessManagers)Simulator::Schedule(GetSifs(), &ChannelAccessManager::NotifyMaybeCcaBusyStartNow, m_channelAccessManager, Seconds (1)); 
          ...
          if (it->first == Mac48Address::GetBroadcast ())
           {
	     ru = ruNumber+ ruNum->GetInteger (1, it->second);
               //ruNumber+= it->second;//nScheduledRUs + nUORA rus
	     m_noSlots -= it->second;
...
Time
MacLow::GetAckDuration (WifiTxVector ackTxVector, uint16_t staId) const
{
 ...
 // if(ackTxVector.GetPreambleType () == WIFI_PREAMBLE_HE_MU)
      //return m_phy->CalculateTxDuration (m_dlMuPsdus, ackTxVector, m_phy->GetPhyBand ());
  return m_phy->CalculateTxDuration (GetAckSize (), ackTxVector, m_phy->GetPhyBand (), staId);
}
...
MacLow::GetCtsTxVector (Mac48Address to, WifiMode rtsTxMode) const
{
  //NS_ASSERT (!to.IsGroup ());
    std::cout<<"Sending CTS for packet destinating "<<to<<std::endl;
  //Xyct: to used to determine greenfield
  //set that to true in case of Group
  WifiMode ctsMode = GetControlAnswerMode (rtsTxMode);
  WifiTxVector v;
  v.SetMode (ctsMode);
  v.SetPreambleType (GetPreambleForTransmission (ctsMode.GetModulationClass (), m_stationManager->GetShortPreambleEnabled (), to.IsGroup()?true:m_stationManager->UseGreenfieldForDestination (to)));
...
MacLow::GetAckTxVector (Mac48Address to, WifiMode dataTxMode) const
{
  ...
  v.SetTxPowerLevel (m_stationManager->GetDefaultTxPowerLevel ());
  v.SetChannelWidth (m_phy->GetChannelWidth()/*GetChannelWidthForTransmission (ackMode, m_phy->GetChannelWidth ())*/);//Xyct: to let ack send in the same ru
                                ...
                                uint32_t fragmentSize) const
{
  ...
  if (fragmentSize > 0)
    ...
    WifiTxVector dataTxVector =GetDataTxVector (item);//Xyct: to add mu, separate variable out
  if (m_phy->GetMuMode ())
   {  
     dataTxVector.SetMuMode (m_phy->GetMuMode ());
     dataTxVector.SetRuBits (m_phy->GetRuBits ());
   } 
  txTime += m_phy->CalculateTxDuration (dataSize, dataTxVector, m_phy->GetPhyBand ());
  return txTime;
}
                                 ...
                                 const MacLowTransmissionParameters& params) const
{
  ...
  if (params.MustSendRts ())
    {
      ...
      if (m_phy->GetMuMode ())
       ...
       } //Xyct: this is moved from the frommer method
  ...
  txTime += GetResponseDuration (params, dataTxVector, item->GetHeader ().GetAddr1 ());
...
{
  NS_LOG_FUNCTION (this << psdu << txVector<< Simulator::Now ().GetSeconds ());
  ...
  if(txVector.IsMu()){//Xyct: currently only send the first psdu
      if(txVector.GetPreambleType() == WIFI_PREAMBLE_HE_MU){//DL call this when caching, call m_phy->Send() later
          m_dlMuTxVector = txVector;
          //std::cout<<"DL HE PSDU forwardDown completed"<<std::endl;
      }else{//UL
        WifiConstPsduMap psdus;
        psdus.insert (std::make_pair (staId, psdu)); 
        m_phy->Send(psdus, txVector);
      }
  }else{
...
MacLow::CtsTimeout (void)
{
  ...
  if(m_currentPacket->GetAddr1 (). IsGroup()){
    m_stationManager->ReportMuRtsFailed (&m_currentPacket->GetHeader (0));
  }
  else
  {
...
MacLow::SendRtsForPacket (void)
{
  ...
  duration += GetSifs ();
 	  if (m_phy->GetMuMode ())//Xyct: Before 3.28, there is an outer if. These were in the "else" part.
           {  
  ...
  if (m_txParams.MustWaitNormalAck () && !IsCfPeriod ())
    {
      ...
      Time timerDelay = txDuration + GetSifs () + GetSlotTime ()
                        + m_phy->CalculatePhyPreambleAndHeaderDuration (ackTxVector);//Xyct: since already multi-ed by 9, ackTxVector not set mu.
     //if (m_phy->GetMuMode ())//Xyct: original hack multi-ed "m_ackTimeout" by 9. I regard it as all left
      //{
        timerDelay = HeRu::GetNRus(m_phy->GetChannelWidth(),HeRu::RU_26_TONE) * timerDelay;
      //}
  ...
  else if (m_txParams.MustWaitBlockAck ())
    {
     ...
     if (m_phy->GetMuMode ())//Xyct: original hack multi-ed "m_blcokAckTimeout" by 9. I regard it as all left
      {
        timerDelay = txDuration + 9 * timerDelay;
      }
      else 
      {
        timerDelay = txDuration + timerDelay;
      }
  ...
  else if (m_txParams.HasNextPacket ())
    {
     NS_LOG_DEBUG ("HasNextPacket");
      ...
      m_waitIfsEvent = Simulator::Schedule (delay, &MacLow::WaitIfsAfterEndTxFragment, this);
           ...
           m_currentTxop->GetTxopLimit ().IsStrictlyPositive () && m_currentTxop->GetTxopRemaining () > GetSifs ())
   {
     NS_LOG_DEBUG ("Xyct: IsQosData && HasTxop when calculate txTime in maclow");
      Time delay = txDuration + GetSifs ();
...
MacLow::SendDataPacket (void)
{
  ...
  if (!m_currentPacket->GetHeader(0).IsTF())//Xyct: for convenience's sake, be hacking
  {
          if (!IsCfPeriod ())
            {
...
void
MacLow::CacheAckAfterData (Mac48Address source, Time duration, WifiMode dataTxMode, double dataSnr, std::pair<uint16_t,HeMuUserInfo> infoPair)
{
    //Prepare for another round of contention
  NS_LOG_FUNCTION (this << Simulator::Now ().GetSeconds ());
  uint16_t staId = infoPair.first;
  ...
  m_dlMuTxVector.SetHeMuUserInfo(staId, infoPair.second);
  duration -= MicroSeconds(844);
  duration -= GetAckDuration (m_dlMuTxVector, staId);
  duration -= GetSifs ();
  duration = MicroSeconds(90);
  ...
  m_dlMuPsdus.insert(std::make_pair(staId, psdu));
}
