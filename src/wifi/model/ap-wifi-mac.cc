...
ApWifiMac::SendTriggerFrame(bool flag)
{
  ...
  hdr.SetDsNotFrom ();
  hdr.SetDsNotTo ();
  hdr.SetDuration(GetTfDuration() * m_low->GetSlotTime());
  Ptr<Packet> packet = Create<Packet> ();
  ...
  std::cout<<"AP "<<GetAddress() <<" queueing TF, time = "<< Simulator::Now ().GetMicroSeconds ()<<std::endl;
  ...
  //m_channelAccessManager->NotifyMaybeCcaBusyStartNow (NanoSeconds(1));
  if (GetQosSupported ()){
...
ApWifiMac::DoInitialize (void)
{
  ...
  if (m_enableBeaconGeneration)
    ...
    NS_LOG_DEBUG ("Scheduling initial Trigger Frame beacon for access point " << GetAddress () << " at time "<< 300000);
    Simulator::Schedule (MilliSeconds(1000), &ApWifiMac::StartMuModeUplink, this);

  RegularWifiMac::DoInitialize ();
  if (GetQosSupported ()){
      for(int i=0;i<8;++i)
    m_edca[QosUtilsMapTidToAc (i)]->SetTfAccessGrantCallback (MakeCallback (&ApWifiMac::TriggerFrameExpire, this));
  }else{
    m_txop->SetTfAccessGrantCallback (MakeCallback (&ApWifiMac::TriggerFrameExpire, this));
  }
}
